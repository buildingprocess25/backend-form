...
const emailTemplates = {
  ...
  h10: {
    ...
    getHtmlBody: (data) => {
      ...
      const taskType = isSerahTerima
        ? "Serah Terima"
        : getNextTaskType("h10", data.kategori_lokasi);
  ...
  perpanjangan_spk_approval: {
    getSubject: (data) =>
      `[PERLU PERSETUJUAN] Perpanjangan SPK untuk Toko: ${data.nomor_ulok}`,
    ...
    getHtmlBody: (data) => `
        <div style="font-family: Arial, sans-serif; line-height: 1.6;">
          ...
          <ul>
            ...
            <li><strong>Tanggal SPK Akhir:</strong> ${formatIndoDate(
              data.tanggal_spk_akhir
            )}</li>
            <li><strong>Pertambahan Hari:</strong> ${
              data.pertambahan_hari
            } hari</li>
            <li><strong>Tanggal SPK Akhir (Setelah Perpanjangan):</strong> ${formatIndoDate(
              data.tanggal_spk_akhir_baru
            )}</li>
            ...
            <li><strong>Dokumen Terlampir:</strong> <a href="${
              data.pdfUrl
            }" target="_blank">Lihat PDF</a></li>
          </ul>
          ...
          <table cellspacing="0" cellpadding="0">
            <tr>
              <td>
                <a href="${
                  data.approveUrl
                }" style="background-color: #28a745; color: white; padding: 12px 20px; text-decoration: none; border-radius: 5px; display: inline-block; text-align: center;">SETUJU</a>
              </td>
              <td style="padding-left: 10px;">
                <a href="${
                  data.rejectUrl
                }" style="background-color: #dc3545; color: white; padding: 12px 20px; text-decoration: none; border-radius: 5px; display: inline-block; text-align: center;">TOLAK</a>
              </td>
            </tr>
          </table>
          <br/>
  ...
  perpanjangan_spk_notification: {
    getSubject: (data) =>
      `[${data.status_persetujuan}] Perpanjangan SPK untuk Toko: ${data.nomor_ulok}`,
    ...
    getHtmlBody: (data) => {
      ...
      if (isApproved && data.kontraktor_nama)
        namaPenerima.push(`<strong>${data.kontraktor_nama}</strong>`);
      if (isApproved && data.manager_nama)
        namaPenerima.push(`<strong>${data.manager_nama}</strong>`);

      const sapaan =
        namaPenerima.length > 0
          ? `<p>Yth. Bapak/Ibu ${namaPenerima.join(", ")},</p>`
          : `<p>Pemberitahuan,</p>`;
      ...
      const statusWarnaFinal = isApproved
        ? "green"
        : isRejected
        ? "red"
        : "black";
      ...
      if (
        dateToParse &&
        !dateToParse.includes("Z") &&
        !dateToParse.includes("+")
      ) {
        dateToParse = dateToParse.replace(" ", "T") + "+07:00";
      }

      const waktuPersetujuanFormatted = dateToParse
        ? new Date(dateToParse).toLocaleString("id-ID", {
            day: "numeric",
            month: "2-digit",
            year: "numeric",
            hour: "2-digit",
      ...
      return `
        <div style="font-family: Arial, sans-serif; line-height: 1.6;">
            ${sapaan}
            <p>Permintaan perpanjangan SPK untuk <strong>Nomor Ulok ${
              data.nomor_ulok
            }</strong> telah direspon.</p>
            <ul>
                <li><strong>Status:</strong> <strong style="color:${statusWarnaFinal};">${
        data.status_persetujuan
      }</strong></li>
                <li><strong>Ditinjau Oleh:</strong> ${
                  data.disetujui_oleh || "N/A"
                }</li>
                ...
                <li style="margin-top: 10px;"><strong>Tanggal SPK Akhir:</strong> ${formatIndoDate(
                  data.tanggal_spk_akhir
                )}</li>
                <li><strong>Pertambahan Hari:</strong> ${
                  data.pertambahan_hari
                ...
                <li><strong>Tanggal SPK Akhir (Setelah Perpanjangan):</strong> ${formatIndoDate(
                  data.tanggal_spk_akhir_setelah_perpanjangan
                )}</li>
                <li style="margin-top: 10px;"><strong>Dokumen Terlampir:</strong> <a href="${
                  data.link_pdf
                }" target="_blank">Lihat PDF</a></li>
            </ul>
  ...
  materai_upload: {
    getSubject: (data) =>
      `Dokumen Final RAB Penawaran Termaterai - ${data.ulok}`,
    getRecipients: (data) => data.recipients || [],
    getHtmlBody: (data) => {
      const sapaanNama =
        data.recipientNames && data.recipientNames.length > 0
          ? data.recipientNames.map((n) => `<strong>${n}</strong>`).join(", ")
          : "Tim Terkait";

      const dokumenUrl = data.dokumenUrl || data.pdfUrl || "#";
      const sphUrl = data.sphUrl || "";
      ...
      <div style="font-family: Arial, sans-serif; line-height: 1.6;">
        ...
        <ul>
          <li><strong>Tanggal Upload:</strong> ${
            data.tanggal_upload
              ? new Date(data.tanggal_upload).toLocaleString("id-ID", {
                  timeZone: "Asia/Jakarta",
                })
              : "N/A"
          }</li>
          ...
          <li><strong>Lingkup Kerja:</strong> ${
            data.lingkup_kerja || "N/A"
          }</li>
        </ul>
        <p>Silakan lihat dokumen yang diunggah melalui tautan di bawah ini:</p>
        <ul>
          <li>
            📄 <a href="${dokumenUrl}" target="_blank">
              Rekapitulasi RAB Termaterai (PDF)
            </a>
          </li>
          ${
            sphUrl
              ? `<li>📄 <a href="${sphUrl}" target="_blank">SPH Termaterai (PDF)</a></li>`
              : ""
          }
        </ul>
